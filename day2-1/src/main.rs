...
Game 100: 16 blue, 12 red, 3 green; 2 green, 7 blue; 5 blue, 4 green; 10 blue, 6 red, 6 green; 5 red, 12 blue, 2 green; 9 red, 12 blue, 11 green";
    ...
    let total: u32 = input.lines().filter_map(|line| {
        let (game_lbl, game_content) = line.split_once(": ").unwrap();
        let game_num = game_lbl.split_once(" ").unwrap().1.parse::<u32>().unwrap();

        let sets = game_content.split("; ");

        if sets.into_iter().all(is_set_possible) {
...
fn is_set_possible(input: &str) -> bool {
    for set in input.split(", ") {
        let (count_str, color) = set.split_once(" ").unwrap();
