...
eight6twojtzlvlhgjncvx";
    ...
    for line in input.lines() {
        ...
        let last_num = get_first_number(line.chars().rev().collect::<String>().as_str());
        let two_digit = first_num * 10 + last_num;
        accumulator += two_digit;
    }

    println!("{}", accumulator);
}

fn get_first_number(input: &str) -> u32 {
    for char2 in input.chars() {
        match char2.to_digit(10) {
            Some(m) => return m,
            None => continue
        }
    }
