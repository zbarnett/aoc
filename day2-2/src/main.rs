...
Game 100: 16 blue, 12 red, 3 green; 2 green, 7 blue; 5 blue, 4 green; 10 blue, 6 red, 6 green; 5 red, 12 blue, 2 green; 9 red, 12 blue, 11 green";
    ...
    let total: u32 = input.lines().map(|line| {
        let game_content = line.split_once(": ").unwrap().1;

        get_power(game_content)
    }).sum();
...
fn get_power(game_content: &str) -> u32 {
    let raw_pulls = game_content.split("; ");

    let pulls: Vec<Pull> = raw_pulls.map(get_pull).collect();

    let red_max = pulls.iter().map(|x| x.red).max().unwrap();
    let green_max = pulls.iter().map(|x| x.green).max().unwrap();
    let blue_max = pulls.iter().map(|x| x.blue).max().unwrap();

    red_max as u32 * green_max as u32 * blue_max as u32
}

fn get_pull(input: &str) -> Pull {
    let hash_map: HashMap<&str, u8> = input.split(", ").map(|x| { 
    ...
    Pull {
        red: hash_map.get("red").cloned().unwrap_or(0),
        green: hash_map.get("green").cloned().unwrap_or(0),
        blue: hash_map.get("blue").cloned().unwrap_or(0),
    }
}